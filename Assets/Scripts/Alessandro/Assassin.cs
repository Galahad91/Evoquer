...
public class Assassin : Characters
{
	...
	void Update()
	{
		...
		if (gestore.Turni % 2 == 0 ) 
		{
			...
			if (gestore.IsFaseCombattimento () == true && gameObject.layer == LayerMask.NameToLayer("Player 2") && currentHealth > 0) 
			{

				Avvia ();
                anim.SetBool("isWalking", true);
                //StartCoroutine(Passi(2, 3));
		...
		else 
		{
			...
			if (gestore.IsFaseCombattimento () == true && gameObject.layer == LayerMask.NameToLayer("Player 1") && currentHealth > 0) 
			{

				Avvia ();
                anim.SetBool("isWalking", true);
                //StartCoroutine(Passi(4, 5));
                //Settiamo un nuovo target
                /*if (AttivoSetTarget == true) 
				{
    ...
    IEnumerator Passi(int suono1,int suono2)
    {
        yield return new WaitForSeconds(0.5f);
        gestore.GetComponent<Musica>().RiproduciSuono(suono1);
        yield return new WaitForSeconds(0.5f);
        gestore.GetComponent<Musica>().RiproduciSuono(suono2);
    }
	...
	public void DamageTaken_Assassin (float damage, int color)
	{
	...
	IEnumerator Explosion()
	{
        ...
        gestore.GetComponent<Musica>().RiproduciSuono(6);
        yield return new WaitForSeconds (2f);
		Destroy (this.gameObject);

	}
	//TEST
	IEnumerator ParticleDamage(int color)
	{
        Color particleEffect;

        switch (color)
        {
            case 1:
                particleEffect = new Color(217, 232, 255, 255);//blu
                break;
            case 2:
                particleEffect = new Color(308, 206, 212, 255);//viola
                break;
            case 3:
                particleEffect = new Color(28, 238, 248, 255);//arancione
                break;
            default:
                particleEffect = new Color(115, 236, 238, 255);//verde
                break;
        ...
        ParticleSystem.MainModule main = startingColor.main;
        main.startColor = new ParticleSystem.MinMaxGradient(particleEffect);
