...
public class IA_Torre : MonoBehaviour 
{
	...
	//[System.Serializable]
	public  List<GameObject> Lista_Nemici;
	...
	private void Attacco()
	{
		...
		for (int i = 0; i < Lista_Nemici.Count; i++) 
		{
			...
			if (Lista_Nemici[i] != null) 
			{
                ...
                //Audio Torre Player 1
                if (gameObject.layer == LayerMask.NameToLayer("Player 1"))
                {
                    gestore.GetComponent<Musica>().RiproduciSuono(9);
                }
                //Audio Torre Player 2
                if (gameObject.layer == LayerMask.NameToLayer("Player 2"))
                {
                    gestore.GetComponent<Musica>().RiproduciSuono(8);
                }
	...
	private void DistruggiTorre()
	{
		...
		casella.tag = "Libera";
        //Destroy (gameObject);
        StartCoroutine(Crumble(gameObject));

	}

    IEnumerator Crumble(GameObject torre)
    {
        // View.transform.DOMove(nexus.transform.position, 1f);

        torre.transform.DOShakeRotation(1f, 10);
        torre.transform.DOMoveY(-16f, 2f);
        torre.transform.GetChild(0).GetComponent<ParticleSystem>().Play();
        yield return new WaitForSeconds(2f);
        Destroy(torre);

    }

	private IEnumerator  PulisciLista()
	{
