...
public class GestoreGioco : MonoBehaviour {
	...
	void Start()
	{
        ...
        //gameObject.GetComponent<Musica>().RiproduciSuono(1);
	}
	...
	void Update()
    {
        ...
        //VICTORY!!
       if(Nexus_Player1.GetComponent<Structures>().currentHealth <= 0)
       {
            string playerWinner = "P2 WINS";
            //Vittoria
            //Victory.SetActive(true);
            StartCoroutine(Winner(Victory,playerWinner));
       }

        if (Nexus_Player2.GetComponent<Structures>().currentHealth <= 0)
        {
           string playerWinner = "P1 WINS";
            //Vittoria
            //Victory.SetActive(true);
            StartCoroutine(Winner(Victory, playerWinner));
        }
        ...
        if (FaseCombattimento == true) 
		{
			...
			if (timer >= Durata_Turno && Nexus_Player1.GetComponent<Structures>().currentHealth > 0 && Nexus_Player2.GetComponent<Structures>().currentHealth > 0)
            {
				timer = 0;
				//Feedback cambioturno
				CambioTurno ();
	...
	#region COROUTINE
		...
		//Feedback del Cambio turno
		IEnumerator PlayerTurn ()
		{
			if (Turni % 2 == 0)
			{
			turn.transform.DOLocalMoveY (90f, 1f);
			player2Image.transform.DOLocalMove(new Vector3(0f,0f,0f),1f);
			yield return new WaitForSeconds (2f);
			turn.transform.DOLocalMoveY (400f, 1f);
			player2Image.transform.DOLocalMove(new Vector3(232f,-378f,0f),1f);
			//CambioTurno ();
			 yield return null;		
			}
			...
			{
				turn.transform.DOLocalMoveY (90f, 1f);
				player1Image.transform.DOLocalMove(new Vector3(0f,0f,0f),1f);
				yield return new WaitForSeconds (2f);
				turn.transform.DOLocalMoveY (400f, 1f);
			    player1Image.transform.DOLocalMove(new Vector3(-224,-378f,0f),1f);
			   // CambioTurno ();
				yield return null;	
			}
		}
        ...
        IEnumerator Winner(GameObject victory, string winner)
        {
		   ...
		   if(winner == "P2 WINS")
			{
			  ...
			  victory.transform.GetChild(0).GetComponent<Image>().sprite = p2Wins;
			...
			if(winner == "P1 WINS")
			{
				...
				victory.transform.GetChild(0).GetComponent<Image>().sprite = p1Wins;
	...
	#region METODI_GAMEPLAY
		...
		{
          //if(Nexus_Player1.GetComponent<Nexus>().currentHealth > 0 && Nexus_Player1.GetComponent<Nexus>().currentHealth > 0)
         // {
